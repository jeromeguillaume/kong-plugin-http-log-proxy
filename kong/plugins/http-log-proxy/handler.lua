...
local function send_entries(conf, entries)
  ...
  kong.log.debug(fmt("http-log-proxy sent data log server, %s:%s HTTP status %d",
...
-- previous queue implementation.  This ensures that http-log-proxy instances that
...
function HttpLogProxy:log(conf)
  ...
  local queue_conf = Queue.get_plugin_params("http-log-proxy", conf, make_queue_name(conf))
